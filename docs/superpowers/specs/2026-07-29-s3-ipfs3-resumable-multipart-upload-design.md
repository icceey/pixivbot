...
`ArchiveArtifacts` gains a separate `.zip.uploads` directory and a dedicated `remove_upload_state()` operation. The existing download multipart cleanup must continue to remove only `.zip.part` and `.zip.parts`; in particular, startup orphan cleanup must preserve `.zip.uploads` while its final ZIP belongs to an active or retryable queue row. A durable cleanup gate applies before any local upload-state or whole-family deletion: derive `.zip.uploads` from the entry ZIP, delete normally when it is absent, but when it exists require the configured S3/ipfS3 Abort uploader to Abort every safely identifiable matching manifest session successfully. No uploader or an Abort error retains the complete local family. This gate covers successful Telegraph cleanup, upload cancellation/permanent failure, publish completion/permanent failure/cancellation, startup orphan cleanup, and download/background cleanup; publish performs it before marking the row done. `NoSuchUpload` is already a successful provider Abort result.
...
Known active sessions are aborted when they are deliberately replaced. The shared cleanup gate deterministically scans direct `.zip.uploads/*.json` manifests with the configured provider-specific S3/ipfS3 uploader; `NoSuchUpload` is already clean, and one failure does not stop later Abort attempts. An Abort error blocks deletion so the manifests retain the sole upload IDs for a later retry or startup cleanup. The optional Abort uploader is wired independently from image uploading into both upload and publish workers; Pixi, Catbox, or absent uploader configurations pass `None` and therefore cannot use the trait default no-op as a success signal. Malformed, unreadable, identity-mismatched, or otherwise unrecoverable manifests are never aborted against the current endpoint. Those remote sessions cannot be safely discovered without ListMultipartUploads and remain subject to the object store's incomplete-multipart lifecycle policy.
