...
| `src/main.rs` | Build the EH image uploader once when EH is enabled and either Telegraph is available or the provider is S3/ipfS3; share only S3/ipfS3 Abort capability with upload/publish/startup cleanup, and pass `None` for other providers. |
